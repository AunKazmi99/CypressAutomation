...
describe('Auto Login', function () {
    it('My First Test Case', function () {
        cy.LoginAPI().then(function () {
            ...
            cy.task('excelToJsonConverter', Cypress.config('fileServerFolder') + '/cypress/downloads/order-invoice_aun.xlsx').then(function(result) {
                cy.log(result.data[1].A)
                expect(productName).to.equal(result.data[1].B)
            })
            cy.readFile(Cypress.config('fileServerFolder') + '/cypress/downloads/order-invoice_aun.xlsx').then(function(text) {
